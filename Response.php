<?php
	...
	if ($trandata != "")
	{
		...
		if ($returnValue == 1)
		{
			...
			{
				echo "REDIRECT=https://admin.fitgate.live/PG/approved.php";
			}
			else if ($Pipe->getresult() == "NOT CAPTURED" || $Pipe->getresult() == "CANCELED" || $Pipe->getresult() == "DENIED BY RISK" || $Pipe->getresult() == "HOST TIMEOUT")
			{
				...
				echo "REDIRECT=https://admin.fitgate.live/PG/declined.php";
			}
			else
			{
				//Unable to process transaction temporarily. Try again later or try using another card.
                echo "REDIRECT=https://admin.fitgate.live/PG/err-response.php";
			}
		}
		else
		{
